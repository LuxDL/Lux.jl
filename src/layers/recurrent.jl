...
``g = tanh(W_{ig} x + W_{hg} \\times h_{prev} + b_{g})``
...
* `weight_ih`: Concatenated Weights to map from input space ``\\left\\{ W_{ii}, W_{if}, W_{ig}, W_{io} \\right\\}``.
* `weight_hh`: Concatenated Weights to map from hidden space ``\\left\\{ W_{hi}, W_{hf}, W_{hg}, W_{ho} \\right\\}``
* `bias`: Bias vector
