...
function BatchNorm(chs::Int, activation=identity; init_bias=zeros32, init_scale=ones32,
  ...
  return BatchNorm{affine, track_stats, typeof(activation), typeof(init_bias),
                   typeof(init_scale), typeof(epsilon)}(activation, epsilon, momentum, chs,
                                                        init_bias, init_scale)
end
...
function (BN::BatchNorm)(x::AbstractArray{T, N}, ps, st::NamedTuple) where {T, N}
  x_normalized, xmean, xvar = normalization(x, st.running_mean, st.running_var, ps.scale,
                                            ps.bias, BN.activation, collect([1:(N - 2); N]),
                                            st.training, BN.momentum, BN.epsilon)
...
function (BN::BatchNorm{affine, track_stats})(x::Union{CuArray{T, 2}, CuArray{T, 4},
  ...
  res = BN.activation.(batchnorm(affine ? ps.scale : nothing, affine ? ps.bias : nothing, x,
                                 running_mean2, running_var2, BN.momentum; eps=BN.epsilon,
...
function LayerNorm(shape::NTuple{N, <:Int}, activation=identity; epsilon::T=1.0f-5,
  ...
  activation = NNlib.fast_act(activation)
  return LayerNorm{affine, typeof(activation), N, T, typeof(init_bias), typeof(init_scale),
                   typeof(dims)}(shape, activation, epsilon, init_bias, init_scale, dims)
end
...
@generated function (l::LayerNorm{affine, F1})(x::AbstractArray, ps,
  ...
  if affine
    if F1 == typeof(identity)
      push!(calls,
            :(return ps.scale .* (x .- _mean) ./ sqrt.(_var .+ l.epsilon) .+ ps.bias, st))
    else
      push!(calls,
            :(return l.activation.(ps.scale .* (x .- _mean) ./ sqrt.(_var .+ l.epsilon) .+
                                   ps.bias), st))
    end
  else
    if F1 == typeof(identity)
      ...
      push!(calls, :(return l.activation.((x .- _mean) ./ sqrt.(_var .+ l.epsilon)), st))
    end
  end
  return Expr(:block, calls...)
end
