...
module NilSizePropagation
    ...
    function Base.convert(
            ::Type{Nil},
            ::Union{StaticBool{N}, Static.StaticFloat64{N}, Static.StaticInt{N}}
        ) where {N}
    ...
    for f in [
            :copy, :zero, :one, :oneunit, :+, :-, :abs, :abs2, :inv,
    ...
    function Base.promote_rule(
            ::Type{Nil}, ::Type{ForwardDiff.Dual{T, V, Tag}}
        ) where {T, V, Tag}
        throw(ArgumentError(NIL_DUAL_ERROR_MSG))
    end
    ...
    function Base.similar(
            bc::Broadcast.Broadcasted{Broadcast.ArrayStyle{NilArray}},
            ::Type{ElType}
        ) where {ElType}
    ...
    for N in (3, 4, 5)
        @eval function NNlib.conv!(
                y::AnyNilArray{$N}, x::AbstractArray{<:Number, $N},
    ...
    function LuxLib.Impl.batchnorm(
            ...
            ::StaticBool, act::F, ::Number, ::Number
        ) where {N, F}
        return x, rμ, rσ²
    end

    function LuxLib.Impl.groupnorm(
            x::AnyNilArray{N}, ::Optional{<:AbstractVector},
            ::Optional{<:AbstractVector}, ::Int, act::F, ::Number
        ) where {N, F}
        return x
    end

    function LuxLib.Impl.normalization(
            x::AnyNilArray, rμ::Optional{<:AbstractVector},
            rσ²::Optional{<:AbstractVector}, ::Optional{<:AbstractVector},
            ::Optional{<:AbstractVector}, _, ::StaticBool, __, ___, act::F
        ) where {F}
        return x, rμ, rσ²
    end
    ...
    function LuxLib.Impl.affine_normalize(
            ::F, x::AnyNilArray, ::Numeric, ::Numeric,
