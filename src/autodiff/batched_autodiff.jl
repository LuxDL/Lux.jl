...
function CRC.rrule(
        ::typeof(batched_jacobian_internal), f::F,
        backend::AbstractADType, x::AbstractArray, y
    ) where {F}
...
function CRC.rrule(
        cfg::RuleConfig{>:HasReverseMode}, ::typeof(batched_jacobian_internal),
        f::F, backend::AbstractADType, x::AbstractArray, y
    ) where {F}
    grad_fn = let cfg = cfg
        (f̂, x, args...) -> begin
...
function CRC.rrule(
        ::typeof(batched_jacobian_internal), f::F,
        backend::AbstractADType, x::AbstractArray
    ) where {F}
    return CRC.rrule_via_ad(
...
@views function batched_forwarddiff_jacobian(
        f::F, x::AbstractMatrix{T}, ::Type{Tag},
