...
BEGINNER_TUTORIALS = []
INTERMEDIATE_TUTORIALS = ["NeuralODE/mnist_classification.jl"]
ADVANCED_TUTORIALS = []
MAPPING = Dict(
    "beginner" => [],
    "intermediate" => [],
...
for (d, paths) in (("beginner", BEGINNER_TUTORIALS), ("intermediate", INTERMEDIATE_TUTORIALS), ("advanced", ADVANCED_TUTORIALS))
    for p in paths
        Literate.markdown(get_example_path(p), joinpath(OUTPUT, d, dirname(p)); credit = false)
