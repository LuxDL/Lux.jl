...
@testset "Parallel" begin
    ...
    @testset "$mode" for (mode, aType, dev, ongpu) in MODES
        ...
        @testset "layer connection" begin
            ...
            struct ConcatLayerParallel <: Lux.AbstractLuxLayer end
            Lux.initialparameters(::Random.AbstractRNG, ::ConcatLayerParallel) =
                NamedTuple()
            Lux.initialstates(::Random.AbstractRNG, ::ConcatLayerParallel) = NamedTuple()
            (::ConcatLayerParallel)(x_tuple, ps, st) = (vcat(x_tuple...), st)

            fusion_layer = ConcatLayerParallel()
...
@testset "BranchLayer" begin
    ...
    @testset "$mode" for (mode, aType, dev, ongpu) in MODES
        ...
        @testset "fusion layer" begin
            ...
            # Create a simple fusion layer that concatenates its tuple inputs
            struct ConcatLayerBranch <: Lux.AbstractLuxLayer end
            Lux.initialparameters(::Random.AbstractRNG, ::ConcatLayerBranch) = NamedTuple()
            Lux.initialstates(::Random.AbstractRNG, ::ConcatLayerBranch) = NamedTuple()
            (::ConcatLayerBranch)(x_tuple, ps, st) = (vcat(x_tuple...), st)

            fusion_layer = ConcatLayerBranch()
...
@testset "Coupling Type Instability: #416" begin
