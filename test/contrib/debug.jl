using Lux, Test, Logging, Zygote, ChainRulesCore
...
@testset "$mode: Dimension Mismatch" for (mode, aType, device, ongpu) in MODES
    ...
    @test_logs min_level=Logging.Error Zygote.gradient(sum ∘ first ∘ model_fixed_debug,
        x, ps, st)
end
...
@testset "$mode: NaN Debugging" for (mode, aType, device, ongpu) in MODES
    ...
    @test_throws DomainError only(Zygote.gradient(ps -> sum(first(model_debug(x, ps, st))),
        ps))
