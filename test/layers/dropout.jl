...
@testset "Dropout" begin
  ...
  test_gradient_correctness_fdm(x -> sum(layer(x, ps, st)[1]), x; atol=1.0f-3, rtol=1.0f-3)
...
@testset "VariationalHiddenDropout" begin
  ...
  test_gradient_correctness_fdm(x -> sum(layer(x, ps, st)[1]), x; atol=1.0f-3, rtol=1.0f-3)
  test_gradient_correctness_fdm(x -> sum(layer(x, ps, st_)[1]), x; atol=1.0f-3, rtol=1.0f-3)
