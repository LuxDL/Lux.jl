...
function train_step(ddim::DenoisingDiffusionImplicitModel{T},
        images::AbstractArray{T, 4}, rng::AbstractRNG, ps,
        st::NamedTuple, opt_st::NamedTuple) where {T <: AbstractFloat}
...
function save_checkpoint(ps, st, opt_st, output_dir, epoch)
    ...
    cpu_dev = cpu_device()
    return bson(
        path, Dict(:ps => cpu_dev(ps), :st => cpu_dev(st), :opt_st => cpu_dev(opt_st)))
end

function save_as_png(
        images::AbstractArray{T, 4}, output_dir, epoch) where {T <: AbstractFloat}
...
@main function main(; dataset_dir::String, epochs::Int=1, image_size::Int=64,
        batchsize::Int=64, learning_rate::Float64=1e-3,
        weight_decay::Float64=1e-4, val_diffusion_steps::Int=3,
        checkpoint_interval::Int=5, output_dir::String="output/train",
        # model hyper params
    ...
    data_loader = DataLoader(ds; batchsize=batchsize, partial=false,
        collate=true, parallel=true, rng=rng, shuffle=true)
    ...
    for epoch in 1:epochs
        ...
        generated_images, _ = generate(ddim, Lux.replicate(rng_gen), # to get inference on the same noises
            (image_size, image_size, 3, 10), val_diffusion_steps, ps, st)
