# # [Normalizing Flows for Density Estimation with RQS Bijector](@id RealNVP-RQS-Tutorial)

# This tutorial demonstrates how to use Lux to train a
# [RealNVP](https://arxiv.org/abs/1605.08803) with RQS (Rational Quadratic Spline) bijectors.
# This is based on the [RealNVP implementation in MLX](https://github.com/ml-explore/mlx-examples/blob/main/normalizing_flow/).
...
# RQS Bijector implementation
@concrete struct RQSAffineBijector <: AbstractBijector
    logits <: AbstractArray
end

function RQSAffineBijector(logits::AbstractArray{T, N}) where {T, N}
    return RQSAffineBijector(logits)
end

function forward_and_log_det(bj::RQSAffineBijector, x::AbstractArray)
    # Normalize input to [0,1] for RQS
    x_norm = clamp.(x, 0, 1)

    # Apply RQS transformation
    y_norm, log_det_rqs = rqs_forward(x_norm, bj.logits)
...
function inverse_and_log_det(bj::RQSAffineBijector, y::AbstractArray)
    # Normalize input to [0,1] for RQS
    y_norm = clamp.(y, 0, 1)

    # Apply RQS inverse transformation
    x_norm, log_det_rqs = rqs_inverse(y_norm, bj.logits)
...
@concrete struct RealNVP <: AbstractLuxContainerLayer{(:conditioners,)}
    ...
    dist_dims::Int
    n_transforms::Int
    use_rqs::Bool  # Flag to choose between affine and RQS
    rqs_k::Int     # Number of bins for RQS
end
...
function RealNVP(; n_transforms::Int, dist_dims::Int, hidden_dims::Int, n_layers::Int, use_rqs::Bool = false, rqs_k::Int = 4)
    if use_rqs
        # For RQS, conditioner outputs 3K+1 parameters per dimension
        out_dims = (3 * rqs_k + 1) * dist_dims
    else
        # For affine, conditioner outputs 2 parameters per dimension
        out_dims = 2 * dist_dims
    end

    conditioners = [
        MLP(dist_dims, hidden_dims, out_dims, n_layers; activation = gelu) for
        _ in 1:n_transforms
    ]
    conditioners = NamedTuple{ntuple(Base.Fix1(Symbol, :conditioners_), n_transforms)}(
        Tuple(conditioners),
    )
    return RealNVP(conditioners, dist_dims, n_transforms, use_rqs, rqs_k)
end
...
function log_prob(l::StatefulRealNVP, x::AbstractArray{T}) where {T}
    ...
    for (mask, conditioner) in Iterators.reverse(zip(l.st.mask_list, smodels))
        if l.model.use_rqs
            bj = MaskedCoupling(mask, conditioner, params -> RQSAffineBijector(params))
        else
            bj = MaskedCoupling(mask, conditioner, params -> AffineBijector(params))
        end
...
function main(;
    ...
    lr::Float64 = 0.0004,
    noise::Float64 = 0.06,
    use_rqs::Bool = true,  # Use RQS bijector
    rqs_k::Int = 4,        # Number of bins for RQS
)
    ...
    model = RealNVP(; n_transforms, dist_dims = 2, hidden_dims, n_layers, use_rqs, rqs_k)
    ps, st = xdev(Lux.setup(rng, model))
    opt = Adam(lr)

    train_state = Training.TrainState(model, ps, st, opt)
    @printf "Total Trainable Parameters: %d\n" Lux.parameterlength(ps)
    @printf "Using %s bijector\n" (use_rqs ? "RQS" : "Affine")
