...
function ImageDataset(folder::String, augmentation_pipeline, normalization_parameters)
    ...
    if istrain
        image_files = vcat(map(
            (x, y) -> joinpath.((x,), y), label_dirs, readdir.(label_dirs))...)
        ...
        remove_files = [
            ...
            "n03529860_11437.JPEG", "n03544143_17228.JPEG",
            "n03633091_5218.JPEG", "n03710637_5125.JPEG", "n03961711_5286.JPEG",
            "n04033995_2932.JPEG", "n04258138_17003.JPEG", "n04264628_27969.JPEG",
    ...
    return ImageDataset(
        image_files, labels, mapping, augmentation_pipeline, normalization_parameters)
end
...
function construct(cfg::DatasetConfig)
    ...
    train_iter = Iterators.cycle(MLUtils.eachobsparallel(
        train_data; executor=ThreadedEx(), buffer=true))
