function Lux.Experimental.single_train_step!(
    ...
    @info "First call to Reactant. Compiling the function!" # TODO: Remove

    compiled_fn = Reactant.compile(
        internal_grad_and_step!, (obj_fn, ts.model, ps, st, st_opt, data, ts.optimizer))

    loss, st_updated, stats = compiled_fn(
        obj_fn, ts.model, ps, st, st_opt, data, ts.optimizer)

    cache = TrainingBackendCache{:Reactant, false}(nothing, (; compiled_fn))
...
function Lux.Experimental.single_train_step!(::AutoReactant, obj_fn::F, data,
    ...
    loss, st_updated, stats = ts.cache.extras.compiled_fn(
        obj_fn, ts.model, ts.parameters, ts.states, ts.optimizer_state, data, ts.optimizer)

    ts_new = Lux.Experimental.TrainState(ts.cache, obj_fn, ts.model, ts.parameters,
        st_updated, ts.optimizer, ts.optimizer_state, ts.step + 1)
