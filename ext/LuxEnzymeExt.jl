...
function Lux.Experimental.compute_gradients(::AutoEnzyme, obj_fn::F, data,
    ...
    _, loss = Enzyme.autodiff(
        Enzyme.ReverseWithPrimal, ts.cache.obj_fn, Active, Const(ts.model),
    ...
    ts_new = __construct_new_trainstate(
        ...
        ts.cache.obj_fn, ts.cache.extras.st_wrap, ts.cache.extras.stats_wrap)
...
function Lux.Experimental.compute_gradients(::AutoEnzyme, obj_fn::F, data,
    ...
    ts_new = __construct_new_trainstate(
        st_wrap[], ts.states, ts, obj_fn, dps, obj_fn, st_wrap, stats_wrap)
