...
function jet_target_modules!(list::Vector{String}; force::Bool=false)
    if JET_TARGET_MODULES[] === nothing || (force && JET_TARGET_MODULES[] !== nothing)
        ...
        @info "JET_TARGET_MODULES is already set to $(JET_TARGET_MODULES[]). No changes \
               made. Use `force=true` to force-set the target modules."
    end
end
...
macro jet(expr, args...)
    ...
    for kwexpr in args
        if Meta.isexpr(kwexpr, :(=))
            ...
            elseif kwexpr.args[1] == :broken
                throw(
                    ArgumentError(
                        "`broken` keyword argument is ambiguous. Use `call_broken` or `opt_broken` instead.",
                    ),
                )
            else
    ...
    ex_opt = JET.call_test_ex(
        :report_opt, Symbol("@test_opt"), vcat(opt_extras, all_args), __module__, __source__
    )
