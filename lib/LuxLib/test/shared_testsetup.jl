using LuxLib, MLDataDevices, LuxTestUtils, StableRNGs, Test

LuxTestUtils.jet_target_modules!(["LuxLib"])

const BLAS_BACKEND = lowercase(get(ENV, "BLAS_BACKEND", "default"))

if parse(Bool, get(ENV, "LOAD_LOOPVECTORIZATION", "true"))
    using LoopVectorization: LoopVectorization
    using Octavian: Octavian
end

if BLAS_BACKEND == "default"
    @info "Using default BLAS backend: OpenBLAS"
elseif BLAS_BACKEND == "appleaccelerate"
    @info "Using AppleAccelerate BLAS backend"
    using AppleAccelerate
elseif BLAS_BACKEND == "blis"
    @info "Using BLIS BLAS backend"
    using BLISBLAS
elseif BLAS_BACKEND == "mkl"
    @info "Using MKL BLAS backend"
    using MKL
else
    error("Unknown BLAS backend: $(BLAS_BACKEND)")
end
