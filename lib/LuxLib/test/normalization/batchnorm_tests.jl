...
function batchnorm_fallback(
        ...
        σ::F = identity, momentum = 0.1f0, epsilon = 1.0f-5
    ) where {F, N}
    y, xm, xv = LuxLib.Impl.normalization(
        x, LuxLib.Utils.remove_tracking(running_mean),
...
function run_batchnorm_testing(gen_f, T, sz, training, affine, track_stats, act, aType)
    ...
    if is_training(training)
        ...
        @test_gradients(
            sumabs2first, batchnorm, x, scale, bias, Constant(rm),
    ...
    @testset "$mode" for (mode, aType, ongpu, fp64) in MODES
        @testset "eltype $T, size $sz, $act $affine $track_stats" for (T, sz, training, affine, track_stats, act) in TEST_BLOCKS[1]
            ...
            run_batchnorm_testing(
                generate_fixed_array, T, sz, training,
...
@testitem "Batch Norm: Mixed Precision" tags = [:normalization] setup = [SharedTestSetup] begin
    @testset "$mode" for (mode, aType, ongpu, fp64) in MODES
        ...
        @test_gradients(
            ...
            Constant(running_var), Val(true), gelu, 0.9, 1.0e-5; atol = 1.0f-3,
