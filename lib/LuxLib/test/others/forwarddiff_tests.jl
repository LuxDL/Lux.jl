using ForwardDiff, Zygote, ComponentArrays, LuxLib, Test, NNlib
    ...
    if LuxTestUtils.ZYGOTE_TESTING_ENABLED[]
        @testset "$(mode): Jacobian Vector Products" for (mode, aType, ongpu, fp64) in MODES
            @testset "$(op)(; flipped = $flipped)" for flipped in (true, false),
                ...
                input_dims = [
                    (2, 4, 2, 1, 3), (4, 4, 1, 3), (4, 4, 3, 2), (4, 1, 3), (4, 3, 2)
                ]
                ...
                @testset "Input Dims: $(in_dims) | Weight Dims: $(w_dims)" for (
                    ...
                    test_jvp_computation(
                        xw -> only(
                            Zygote.gradient(xw -> sum(abs2, op(xw.x, xw.w; flipped)), xw),
                        ),
                        ComponentArray(; x, w),
                        u,
                        ongpu,
            ...
            @testset for op in (meanpool,)
                @testset for (input_dim, kernel_size, stride, pad) in (
                    ...
                    test_jvp_computation(
                        x -> only(
                            Zygote.gradient(x -> sum(op(x, kernel_size; stride, pad)), x),
                        ),
                        x,
                        u,
                        ongpu,
