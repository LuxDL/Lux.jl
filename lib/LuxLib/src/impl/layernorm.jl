...
#       kernel abstractions
function layernorm(
        x::AbstractArray{xT, N}, γ::Optional{<:AbstractArray},
        β::Optional{<:AbstractArray}, act::F, dims, epsilon
    ) where {N, F, xT}
...
function compute_layernorm_dims(
        ::AbstractArray{xT, N}, ::AbstractArray{γT, M},
...
function expand_layernorm_dims(
        ::AbstractArray{xT, N}, γ::AbstractArray{γT, M},
...
function expand_layernorm_dims(
        ::AbstractArray{yT, N}, γ::AbstractArray{γT, N},
        β::AbstractArray{βT, N}, dims
    ) where {yT, γT, βT, N}
    return γ, β
end
