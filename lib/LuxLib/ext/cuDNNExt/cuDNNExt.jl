...
using cuDNN:
    ...
    batchnorm_inference!,
    batchnorm_inference_supported,
    batchnorm_training!,
    batchnorm_training_supported
...
const BatchNormFallbackSignature = Tuple{
    AbstractArray,
    OptionalVector,
    OptionalVector,
    OptionalVector,
...
function batchnorm_cudnn_supported(γ, β, x, rμ, rσ², training, momentum, epsilon)
    ...
    if unsafe_known(training)
        batchnorm_training_supported(
            x, x, γ, β; running_mean=rμ, running_var=rσ², momentum, epsilon
        ) || return false
