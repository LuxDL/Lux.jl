_nfan() = 1, 1 # fan_in, fan_out
_nfan(n) = 1, n # A vector is treated as a n×1 matrix
_nfan(n_out, n_in) = n_in, n_out # In case of Dense kernels: arranged as matrices
_nfan(dims::Tuple) = _nfan(dims...)
_nfan(dims...) = prod(dims[1:(end - 2)]) .* (dims[end - 1], dims[end]) # In case of convolution kernels
_norm_cdf(x::T) where {T} = T(0.5) * (1 + T(erf(x / √2))) # erf often doesn't respect the type
