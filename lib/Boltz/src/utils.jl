...
@inline fast_chunk(x::AbstractArray, h::Int, n::Int, ::Val{dim}) where {dim} = selectdim(x, dim, fast_chunk(h, n))
@inline fast_chunk(x::AbstractArray, ::Val{N}, d::Val{D}) where {N,D} = fast_chunk.((x,), size(x, D) ÷ N, 1:N, d)
...
@inline function normalise(x::AbstractArray, ::typeof(identity); dims=ndims(x), epsilon=ofeltype(x, 1e-5))
    xmean = mean(x, dims=dims)
    xstd = std(x, dims=dims, mean=xmean, corrected=false)
    return @. (x - xmean) / (xstd + epsilon)
end

@inline function normalise(x::AbstractArray, activation; dims=ndims(x), epsilon=ofeltype(x, 1e-5))
    xmean = mean(x, dims=dims)
    xstd = std(x, dims=dims, mean=xmean, corrected=false)
