...
"""
struct MultiHeadAttention{Q,A,P} <: Lux.AbstractExplicitContainerLayer{(:qkv_layer, :attention_dropout, :projection)}
    number_heads::Int
    qkv_layer::Q
    attention_dropout::A
    projection::P
...
function MultiHeadAttention(
    ...
    attention_dropout = iszero(attention_dropout_rate) ? NoOpLayer() : Dropout(attention_dropout_rate)
    projection = Chain(
        Dense(in_planes, in_planes), iszero(projection_dropout_rate) ? NoOpLayer() : Dropout(projection_dropout_rate)
    )
...
function (m::MultiHeadAttention)(x::AbstractArray{T,3}, ps, st) where {T}
    ...
    qkv_reshaped = reshape(qkv, nfeatures ÷ m.number_heads, m.number_heads, seq_len, 3 * batch_size)
    query, key, value = fast_chunk(qkv_reshaped, Val(3), Val(4))

    scale = convert(T, sqrt(size(query, 1) / m.number_heads))
    key_reshaped = reshape(
        permutedims(key, (2, 1, 3, 4)), m.number_heads, nfeatures ÷ m.number_heads, seq_len * batch_size
    )
    query_reshaped = reshape(query, nfeatures ÷ m.number_heads, m.number_heads, seq_len * batch_size)
    attention, st_attention = m.attention_dropout(
        softmax(batched_mul(query_reshaped, key_reshaped) .* scale), ps.attention_dropout, st.attention_dropout
    )

    value_reshaped = reshape(value, nfeatures ÷ m.number_heads, m.number_heads, seq_len * batch_size)
    pre_projection = reshape(batched_mul(attention, value_reshaped), (nfeatures, seq_len, batch_size))
    y, st_projection = m.projection(reshape(pre_projection, size(pre_projection, 1), :), ps.projection, st.projection)
...
function ViPosEmbedding(embedding_size::Int, number_patches::Int; init=(rng, dims...) -> randn(rng, Float32, dims...))
    return ViPosEmbedding(embedding_size, number_patches, init)
end

Lux.initialparameters(rng::AbstractRNG, v::ViPosEmbedding) = (vectors=v.init(rng, v.embedding_size, v.number_patches),)
...
"""
function transformer_encoder(in_planes, depth, number_heads; mlp_ratio=4.0f0, dropout_rate=0.0f0)
        ...
        Chain(
            ...
            SkipConnection(
                Chain(
                    layer_norm(in_planes),
                    Chain(
                        Dense(in_planes => hidden_planes, gelu),
                        iszero(dropout_rate) ? NoOpLayer() : Dropout(dropout_rate),
...
function patch_embedding(
    ...
    return Chain(
        Conv(patch_size, in_channels => embed_planes; stride=patch_size),
...
function vision_transformer(;
    ...
    return Chain(
        Chain(
            patch_embedding(imsize; in_channels, patch_size, embed_planes),
            ClassTokens(embed_planes),
            ViPosEmbedding(embed_planes, number_patches + 1),
            iszero(embedding_dropout_rate) ? NoOpLayer() : Dropout(embedding_dropout_rate),
...
function vision_transformer_from_config(mode::Symbol = :base; kwargs...)
    vit_configs = Dict(
        :tiny => (depth=12, embed_planes=192, number_heads=3),
    ...
    return vision_transformer(;vit_configs[mode]..., kwargs...)
end
