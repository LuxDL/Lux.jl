...
@testset "Loaded Trigger Package" begin
    ...
    if MLDataDevices.functional(OpenCLDevice)
        @info "OpenCL is functional"
        @test gpu_device() isa OpenCLDevice
        @test gpu_device(; force=true) isa OpenCLDevice
    else
        @info "OpenCL is NOT functional"
        @test gpu_device() isa OpenCLDevice
...
@testset "Data Transfer" begin
    ...
    device = gpu_device()
    aType = MLDataDevices.functional(OpenCLDevice) ? CLArray : Array
    rngType =
        MLDataDevices.functional(OpenCLDevice) ? OpenCL.GPUArrays.RNG : Random.AbstractRNG

    ps_xpu = device(ps)
    @test get_device(ps_xpu) isa OpenCLDevice
    @test get_device_type(ps_xpu) <: OpenCLDevice
    ...
    @test get_device(ps_xpu.rng_default) isa OpenCLDevice
    @test get_device_type(ps_xpu.rng_default) <: OpenCLDevice
    @test ps_xpu.rng == ps.rng
    @test get_device(ps_xpu.rng) === nothing
    @test get_device_type(ps_xpu.rng) <: Nothing

    if MLDataDevices.functional(OpenCLDevice)
        @test ps_xpu.one_elem isa CLArray
        @test ps_xpu.farray isa CLArray
    else
    ...
    @testset "IsBits Types" begin
        ...
        struct SimpleBitsOpenCL
            field::Int32
        end

        isbits_array = [SimpleBitsOpenCL(1), SimpleBitsOpenCL(2), SimpleBitsOpenCL(3)]
        isbits_array_xpu = device(isbits_array)

        if MLDataDevices.functional(OpenCLDevice)
            @test isbits_array_xpu isa CLArray{SimpleBitsOpenCL}
            @test Array(isbits_array_xpu) == isbits_array

            # Test transfer back to CPU
            isbits_array_cpu = cpu_device()(isbits_array_xpu)
            @test isbits_array_cpu isa Array{SimpleBitsOpenCL}
            @test isbits_array_cpu == isbits_array
        else
            @test isbits_array_xpu isa Array{SimpleBitsOpenCL}
...
@testset "Functions" begin
    if MLDataDevices.functional(OpenCLDevice)
        ...
        ff_xpu = OpenCLDevice()(ff)
        @test get_device(ff_xpu) isa OpenCLDevice
        @test get_device_type(ff_xpu) <: OpenCLDevice
...
@testset "Wrapper Arrays" begin
    if MLDataDevices.functional(OpenCLDevice)
        x = OpenCLDevice()(rand(10, 10))
        @test get_device(x) isa OpenCLDevice
        @test get_device_type(x) <: OpenCLDevice
        x_view = view(x, 1:5, 1:5)
        @test get_device(x_view) isa OpenCLDevice
        @test get_device_type(x_view) <: OpenCLDevice
    end
end
...
@testset "setdevice!" begin
    if MLDataDevices.functional(OpenCLDevice)
        @test_logs (
            :warn,
            "Support for Multi Device OpenCL hasn't been implemented yet. Ignoring the device setting.",
